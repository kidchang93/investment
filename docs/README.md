...
| [`OPERATIONS.md`](./OPERATIONS.md) | **자동매매 운영 매뉴얼 — 매일 어떻게 돌리나.** 사람이 하는 일 셋, 데몬 시간표, 명령어 사전, 판단자와 집행기가 갈린 이유, 안전장치, 문제 대응, 아직 안 되는 것 |
...
| [`MANUAL.md`](./MANUAL.md) | 초보자용 **웹 화면** 사용 설명서. 화면 네 개(목표/종목/내 계좌/발견) 안내, 안전 장치(게이트·리스크 룰·포지션 관문), 주문과 손익 보는 곳, 남아 있는 후보 판정·최소 보유, 전략 판정 읽는 법, 확인된 값과 어림값 구분, 차단 문구 대처표. ★ **화면에서 주문을 내는 자리와 러너 카드는 2026-09-07에 없어졌다** — 자동매매를 돌리는 법은 `OPERATIONS.md` |
...
- `TRADING_ROADMAP.md`의 백테스트 판정(`no_edge`/`unproven`)은 코드
  ...
  코드 쪽 값이다. **측정에는 축(`CandleAxis`)이 함께 다니고, 러너가 돌던 축
  (`RUNNER_CANDLE_AXIS`)의 측정이 맨 앞에 온다** — 축이 없던 시절 일봉 판정문이 1분봉
  러너를 설명하는 글로 읽혔다. (러너와 판정 화면은 2026-09-07에 지웠다.)

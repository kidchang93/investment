...
| 코드 | 뜻 | 확인 방법 |
|------|------|------|
| `311` | 장전 동시호가 | 2026-07-27 08:53~09:00:10, 20초 간격 22개 표본 |
| `112` | 정규장 | 같은 날 09:00:30 이후 18개 표본 |
| `121` | 마감 동시호가 | 같은 날 14:53부터 20초 간격 90개 표본. 15:20:03에 `112` → `121`로 한 번에 바뀌고 되돌아가지 않았다. 같은 시각 `000660`으로도 확인 |
| 그 외 | `unknown` | 확인 못 함 → 예상체결을 감춘다 |

> `121`을 넣기 전에는 마감 동시호가가 `unknown`으로 떨어져 예상체결이 감춰졌고,
> 그동안 화면 큰 글씨는 **15:19:59에 멈춘 정규장 마지막 체결가**였다. `000660`이
> 1,820,000원(+3.47%)으로 굳어 있는 동안 동시호가는 1,836,000원(+4.38%)에
> 지시되고 있었다 — 16,000원 차이를 화면이 말하지 않았다.
>
> 한 종목만 보고 넣지 않는다. `005930`으로 전환을 잡고 `000660`으로 같은 코드를
> 확인한 뒤에 표에 올렸다.

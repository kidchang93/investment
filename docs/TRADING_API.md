...
#### `ORD_DVSN` 코드 — 공식 샘플이 실제로 말하는 것

KIS 공식 레포(`github.com/koreainvestment/open-trading-api`)의
`examples_user/domestic_stock/domestic_stock_functions.py`를 직접 읽고 정리했다.
**엔드포인트마다 문서화 수준이 다르다** — 한 곳의 목록을 다른 곳에 옮겨 적으면 안 된다.

| 함수 | 필드 | 문서에 적힌 값 |
|------|------|------|
| `order_cash` (주문 전송, 우리가 쓰는 것) | `ord_dvsn` | **아무 목록도 없다.** `[필수] 주문구분`이 전부 |
| `order_rvsecncl` (정정·취소) | `ord_dvsn` | 목록 없음 |
| `order_resv` (예약주문) | `ord_dvsn_cd` | **`00` 지정가 / `01` 시장가 / `02` 조건부지정가 / `05` 장전 시간외** |
| `inquire_credit_psamount` (신용매수가능조회) | `ord_dvsn` | `00`~`07` 전체 (`06` 장후 시간외, `07` 시간외 단일가 포함) |
| `inquire_psbl_order` (매수가능조회) | `ord_dvsn` | `01 : 시장가` 하나만 예시 |

**따라서 `06`·`07`로 주문을 낼 수 있다는 근거는 아직 없다.** 그 두 코드가 나오는
곳은 **매수가능조회**이지 주문 전송이 아니다. 코드 자체는 KIS 어휘에 실재하지만,
`order-cash`가 받아 주는지는 **이 자료로는 확인되지 않는다** — 실제로 넣어 보기
전까지는 모른다.

**반면 예약주문의 `05`(장전 시간외)는 문서에 못 박혀 있다.** 우리는
`ORD_DVSN_CD: '00'`으로 고정하고 있어(`rest.ts` 예약주문) 이 선택지를 안 쓴다.
시간외 관련해서 **지금 근거를 갖고 열 수 있는 유일한 문**이다.

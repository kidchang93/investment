...
우리는 `'KRX'` 고정이다(`rest.ts:1808`·`1864`). 거래소를 열려면 **리스크 룰의
시간대(09:00~15:30)부터 손봐야 한다** — NXT는 08:00~20:00이라 지금 룰이 막는다.
...
#### `EXCG_ID_DVSN_CD` — 여기도 엔드포인트마다 다르다

`order_cash`는 **`(KRX)`** 하나만 적었고, `order_rvsecncl`과 신용주문 쪽은
`KRX / NXT(넥스트레이드) / SOR`을 적었다. 우리는 전부 `'KRX'` 고정이다.
**정정·취소가 넓다고 신규 주문도 넓다고 볼 수 없다.**

...
**모름**(`unpricedCount`)으로 센다. 멱등성 경로(`completeClaimedOrder`)가 칸을 따로
UPDATE하는 곳이라 거기에도 넣었다 — 빠지면 판단자·집행기의 해외 주문만 원화로 쌓인다.
...
그래서 *"러너는 서버 메모리에 살고 재시작하면 멈춘다"*(`docs/ARCHITECTURE.md`)는
결정과 **부딪히지 않는다** — 우리 서버가 켜져 있든 말든 상관없다.
...
> 자동매매 쪽에서 이걸 덜 겪게 하는 설정이 **최소 보유 시간**이다(아래 절). 룰 자체는
> 그대로 두고 러너가 매매 횟수를 줄이는 방식이라, 수동 주문은 영향받지 않는다.
...
이라 시장가 주문이 **영원히 0원**으로 쌓였다. 자동매매 러너는 항상 시장가이므로
...
산 지 N분이 안 지났으면 **매도 신호가 나도 그 회차에는 팔지 않는다.**
`POST /api/broker/kis/auto-trader/start`의 `minHoldMinutes`로 정하고, **생략하면
0(끔)**이라 켜기 전에는 지금 동작 그대로다.

| | 값 |
|------|------|
| 범위 | `0`~`390`분. 밖이면 `400`. 상한은 정규장 하루 길이(09:00~15:30)다 |
| 기본값 | `0` — 매도를 미루는 기능이라 기본으로 켜지 않는다 |
| 화면 | 내 계좌 > 자동매매 카드의 `최소 보유 시간 (매도만)` |

**값을 조용히 잘라 맞추지 않는다.** 480을 보냈는데 390으로 깎이면 사용자가 건
것과 러너가 도는 것이 갈린다.

### 왜 리스크 룰에 넣지 않았나

리스크 룰은 **그 계좌의 모든 주문**에 걸린다. 거기 넣으면 수동 매도까지 막히는데,
이건 자동매매가 자기 신호를 보류하는 것이지 주문을 금지하는 것이 아니다. 판단도
러너가 하고 전략은 모른다(안전 순서: 전략 → 러너 → 리스크 룰 → 게이트).
...
**계좌 전체 합산 20건**이라 매수가 먼저 먹으면 그날 못 판다.
...
잣대다 — `blocked`·`rejected`는 접수된 적이 없으므로 포지션이 될 수 없다.

| 상황 | 동작 |
|------|------|
| 매수 기록이 있다 | 그 시각부터 N분을 잰다. 정확히 N분이면 **판다**(경계는 통과) |
| **매수 기록이 없다** | **그대로 판다.** 러너를 켜기 전부터 들고 있던 종목·다른 앱으로 산 종목이 여기다 |
| 조회가 실패했다 | **그대로 판다.** 최소 보유가 적용되지 않았다는 사실을 실행 기록에 남긴다 |

**모를 때 막지 않는 쪽에 두는 유일한 판정이다.** 게이트·개장일·자동매매 상태는
모르면 막는데 여기는 반대다 — 이 레포의 원칙이 「나가는 문이 들어오는 문보다
앞」이고 못 파는 쪽이 훨씬 위험하기 때문이다.

**잰 것은 접수 시각이지 체결 시각이 아니다.** 러너는 늘 시장가라 둘이 거의 같지만,
사람이 낸 지정가가 한참 뒤에 체결되면 실제보다 이르게 잰 것이라 **더 오래 못 파는
쪽**으로 틀린다.
...
```
삼성전자 매도 보류 · 최소 보유 60분을 채우지 못해 이번 회차에는 팔지 않습니다
 · 산 지 12분 · 남은 시간 48분 · <전략이 낸 사유>
```

시작 기록에도 어떤 값으로 돌았는지 남는다(`… · 최소 보유 60분`).

### 위험

**값이 급락해도 그 시간 동안은 자동매매가 팔지 않는다. 지금 손절은 없다.**
손절·익절·트레일링을 넣지 않은 것은 측정에서 이 축(1분봉)에서는 발동하기 전에
신호 반전이 먼저 나고, 발동하면 재진입을 열어 매매만 늘렸기 때문이다
(`docs/USER_FINDINGS.md`). 화면 카드에도 이 두 문장을 함께 적는다.

판정은 `backend/src/trading/minHold.ts`에 순수 함수로 떼어 뒀고 시험은
`minHold.test.ts`다 — 네트워크·DB 없이 경계(정확히 N분)·매수 기록 없음·0분·
매수 신호를 값으로 못 박는다.

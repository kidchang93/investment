...
- [ ] 시세를 여러 번 부르는 기능이면 **상한을 호출 수로** 잡았는가 (종목 수로 잡으면 몇 회가 나가는지 알 수 없다)
- [ ] 그 비용이 **응답과 화면에** 있는가 (`quoteCalls` / 버튼 옆 문구). 모르면 새로고침처럼 눌린다
- [ ] 상한을 넘긴 요청을 **잘라서 200으로** 답하지 않는가 — 잘린 줄 모르면 "이게 전부"로 읽힌다
- [ ] 여럿을 집계할 때 **표본이 몇 개인지**가 응답에 남는가 (명단 N → 찾은 M → 시세 온 K)

### 재는 코드 (측정 스크립트·백테스트)
- [ ] 표본을 **어떻게 골랐는지**가 결과에 남는가. 목록 앞에서 자르지 않았는가 — DB 조회는 검색어가 없으면 `ORDER BY symbol`이라 앞쪽은 종목코드가 작은 순이다
- [ ] 표본에서 **뺀 것을 세어 적는가.** 안 세면 중앙값이 무엇의 중앙값인지 알 수 없다
- [ ] **매매 0건 표본을 0%로 섞지 않았는가.** 전부 마이너스인 분포에서 0은 맨 위다
- [ ] 승률을 적는 자리에 **미청산 표본 수**도 있는가 (`BacktestResult.winRate`는 판 것들만의 값이다)
- [ ] 시험이 **어떤 조건으로 도는가.** 비용·오류처럼 켜야 드러나는 것은 켜고 재는 시험이 따로 있는가 — 승률 시험이 `NO_COSTS`로 돌아 같은 커밋의 비용 계산 버그를 못 잡은 적이 있다

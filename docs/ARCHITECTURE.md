...
러너 자리에서 보던 것 중 둘이 남았다 — **최소 보유 시간**(`minHold.ts`),
**매수 종목의 재무**(`fundamentals.ts`). 둘 다 리스크 룰이 아니다. 리스크 룰은
계좌의 모든 주문에 걸리므로 여기 것을 그쪽으로 옮기면 **수동 주문까지 막는다.**

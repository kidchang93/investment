...
export const TASKS: TaskSpec[] = [
  ...
  {
    /*
     * ★★ **재무를 개장 전에 받아 둔다** (2026-09-03).
     *
     * 사용자가 정했다 — *"회차가 돌게 되면 시간 너무 많이 허비해서 안 되고
     * 분석가를 더 심층적으로 다루는 게 좋을 것 같은데?"*
     *
     * 그래서 분석가의 후보를 14 → 150종목으로 넓혔더니 **한 회차가 10분을
     * 넘겼다.** 병목은 계산이 아니라 종목당 KIS 재무 조회였고, 장중에 그렇게
     * 두들기니 계좌 조회까지 `EGW00201`로 막혔다.
     *
     * 재무는 분기마다 바뀐다 — 장중에 받을 이유가 없다. 미리 채워 두면
     * 분석가는 DB만 읽는다. **실측: 10분+ → 29.7초, 후보 147종목.**
     *
     * ── ★ 개장 전에서 장 마감 뒤로 옮겼다 (2026-09-09) ──────────────────
     *
     * 후보를 900종목으로 넓히면서 첫 채우기가 **49.1분**이 됐다(737종목,
     * 종목당 약 4초 — `getFinancials`가 TR 셋을 부르고 모의는 초당 1건이다).
     * `[08:00, 09:00]` 창에 안 들어가고, 들어간다 해도 개장 전 브리핑·회차와
     * KIS를 두고 다툰다.
     *
     * ★★ **프로세스가 다르면 초당 한도를 함께 지키지 못한다.** `scheduleKisCall`은
     *    한 프로세스 안에서만 줄을 세운다. 실제로 재무를 받는 동안 판단자 화면의
     *    계좌 조회가 `EGW00201`로 막히는 것을 봤다. 그래서 **다른 KIS 작업이
     *    없는 시간**으로 옮긴다 — 마감 정리(15:40~16:20)와 일봉 수집(15:45~17:00)
     *    이 끝난 뒤다.
     *
     * ★ 전날 저녁에 받아도 다음날 아침에 신선하다 — TTL이 7일이다.
     *   평소에는 만료된 것만 받으므로 하루 평균 백여 종목, 몇 분이면 끝난다.
     */
    ...
    daily: true,
    background: true,
    guard: 'warmFinancialCache.ts',
    command: 'cd backend && npx tsx src/scripts/warmFinancialCache.ts',
  },
  {
    name: 'deliberate',
    label: '판단자 소집',
    /*
     * ★★ **개장 뒤로 옮겼다** (2026-09-04). 전에는 [820, ...]이라 창이 열리자마자
     *    08:44에 돌았고, 그 회차는 **구조적으로 신규 후보를 낼 수 없었다.**
     *
     *    그날 판단자가 08:46에 `screenCandidates`를 300종목으로 돌린 결과가
     *    **통과 0 · 호가 없음 300**이었다 — 거래대금 부족도 비용 초과도 아니고
     *    전량이 "호가 없음"이다. 정규장 전이라 호가가 형성되지 않았고,
     *    `verdictFor`가 가장 먼저 그것으로 거른다.
     *
     * ★ `daily: true`라 그 회차가 **그날의 유일한 발굴 기회**였다. 빈손으로
     *   끝나면 하루치 발굴이 통째로 날아간다 — 실제로 그랬다(결정 0건).
     *
     * ★ 개장 점검(보유·손절·층 확인)은 잃지 않는다. 같은 프롬프트가 그것도 한다.
     *   09:10이면 호가가 형성돼 있고 개장 직후 변동성도 한 김 빠진 뒤다.
     */
    window: [910, 1530],
    trading: true,
    daily: true,
  ...
  {
    /*
     * ★★ **하루가 끝날 때 한 번 말한다** (2026-09-03).
     *
     * 사용자가 하루에 두 번 물었다 — *"자동화 켜면 되나?"*, *"자동화 자꾸
     * 꺼지는 것 같은데 브리핑이 또 안 와."* 두 번 다 자동화는 켜져 있었다.
     * 한 번은 실제로 굶어 있었고, 한 번은 **그냥 장이 끝난 것**이었다.
     *
     * 문제는 **"조용한 것"의 뜻을 알 수 없다**는 것이다. 브리핑이 안 오는 것이
     * 정상(마감)인지 고장인지 구별할 방법이 없으면 사람은 매번 물어보거나,
     * 더 나쁘게, 묻지 않고 넘어간다.
     *
     * ★ 첫 실행이 바로 값어치를 보여 줬다 — 모의 서버가 죽은 14:44~15:20에
     *   `open-orders`(미체결 정리)가 한 번도 안 돌았고, 개장 전 `auction-open`도
     *   놓쳤다(자동화를 08:43에 켰는데 그 창은 08:35에 닫힌다).
     *
     * 16:00이면 마감 정리(15:40)와 일봉 수집(15:45)이 시작된 뒤다.
     */
    name: 'daily-report',
    label: '오늘 자동화 정리',
    window: [1600, 1700],
    trading: false,
    daily: true,
    command: 'cd backend && npx tsx src/scripts/dailyAutomationReport.ts --notify',
  },
  {
    /*
     * ★ **KIS 공식 저장소가 우리가 쓰는 곳을 바꿨나** (2026-09-03).
     *
     * 사용자가 짚었다 — *"KIS 깃 포크해온 레포였나. 업데이트 계속 해줘야 될 것
     * 같다."* 포크는 아니지만 **스펙을 숫자로 박아 두고** 있어 우려가 맞다:
     * 실시간 46필드·멀티시세 30종목 상한·주문 구분 코드표가 그렇다.
     *
     * ★ **받아 두고 알린다.** 사용자가 정했다 — *"git pull 해도 상관없고, 만약
     *   큰 변화가 있다면 공지사항에 나올 테니 그걸 확인하면 될 것 같아."* 클론은
     *   읽기 전용 참고 자료라 최신이 낫다. **코드 반영은 사람이 정한다.**
     * ★ 공지사항은 자동으로 못 읽는다(SPA라 서버가 빈 목록만 준다) — 알림에
     *   링크를 실어 사람이 열어 보게 한다.
     * ★ 우리가 쓰는 경로에 변경이 없으면 조용하다. 첫 실행에서 21개 파일이
     *   바뀌었지만 전부 백테스터·전략빌더라 0건이었다 — 전부 알리면 곧 아무도
     *   안 읽는다.
     * ★ 클론이 없으면 그냥 건너뛴다. 이것 때문에 자동화가 실패하면 안 된다.
     */
    name: 'kis-upstream',
    label: 'KIS 스펙 변경 확인',
    window: [1630, 1730],
    trading: false,
    daily: true,
    command: 'cd backend && npx tsx src/scripts/checkKisUpstream.ts --notify',
  },
  {
    /*
     * ★★ **종가 매매 — 판단** (2026-09-07). 사용자가 정했다 — *"장중에는 현재
     *    프로세스대로 매매하고, 장이 끝나고 종가 매매 방식을 도입해보는 것도
     *    좋을 것 같아. 시장 상황을 분석해서 종가에 사서 다음날 매도하는 전략."*
     *
     * ★ **창이 15:00~15:10인 이유.** 회차가 10~15분 걸리므로 15:10에 시작해도
     *   15:25쯤 끝나 아래 집행 창(15:21~15:29)에 닿는다. 더 일찍 열면 종가까지
     *   40분이 남아 그 사이 상황이 바뀌고, 더 늦게 열면 집행 창을 놓친다.
     *
     * ★ **집행을 여기서 하지 않는다.** `deliberate.sh`가 `--close`면 집행을
     *   건너뛰고, 아래 `close-execute`가 동시호가 창에서 낸다.
     */
    name: 'close-judge',
    label: '종가 판단자',
    /*
     * ★★ **창을 넓혔다** (2026-09-09). 처음 [1500,1510]으로 잡았는데 **10분은
     *    재시도할 여유가 없었다.**
     *
     *    2026-09-08 첫 실전에서 15:08:59에 떴다가 3분 만에 죽었다 —
     *    `API Error: Can't reach the API server (ENOTFOUND)`. 코드가 아니라
     *    **네트워크였고**, 실패하면 하트비트를 안 남기므로 다음 루프가 다시
     *    부를 텐데 **그때는 이미 창이 닫혀 있었다.** 그날 종가 매매가 통째로 날아갔다.
     *
     * ★ 15:12까지 열어 둔다. 회차가 10~15분이므로 그때 시작해도 15:27에 끝나
     *   집행 창(15:21~15:30) 안에 든다. 더 늦게까지 열면 집행이 동시호가를 놓친다.
     */
    window: [1455, 1512],
    trading: true,
    daily: true,
    guard: 'deliberate.sh',
    command: 'zsh scripts/deliberate.sh --close',
  },
  {
    /*
     * ★★ **종가 매매 — 집행** (2026-09-07). 장 마감 동시호가는 **15:20~15:30**이다.
     *    그 전에 내면 정규장 주문으로 즉시 체결돼 "종가에 산다"가 성립하지 않는다.
     *
     * ★ **매 2분 돈다.** 판단이 언제 끝날지 모르기 때문이다 — 회차가 없으면
     *   스크립트가 조용히 빠지고, 집행기는 멱등이라 여러 번 불러도 같은 주문이
     *   두 번 나가지 않는다.
     *
     * ★ 15:29에서 끊는다. 15:30을 넘기면 접수가 안 된다.
     */
    name: 'close-execute',
    label: '종가 집행(동시호가)',
    window: [1521, 1530],
    ...
    command: 'zsh scripts/close-execute.sh',
  },
  {
    /*
     * ★★ **종가 매매 — 청산** (2026-09-07). 어젯밤 종가에 산 것을 아침에 판다.
     *
     * ★ **판단이 아니라 규칙이다.** 살 때 이미 "하룻밤"이라고 정했으므로 아침에
     *   다시 판단하면 그때 정한 것을 스스로 지우는 것이 된다 — 손절을 규칙이
     *   집행하는 것과 같은 이유다(`docs/USER_DECISIONS.md`).
     *
     * ★ **09:03에 연다.** 개장 직후 몇 분은 호가가 얇고 변동이 크다. 그렇다고
     *   더 미루면 밤사이 몫을 장중에 되돌려준다 — 이 레포가 잰 개장 갭 되돌림이
     *   바로 그것이다(급등 출발 장중 −1.848%, t −7.46).
     */
    name: 'overnight-exit',
    label: '종가 매매 청산',
    window: [903, 930],
    trading: true,
    daily: true,
    guard: 'exitOvernight.ts',
    command: 'cd backend && npx tsx src/scripts/exitOvernight.ts VTS-ORDINARY --execute',
  },
  {
    /*

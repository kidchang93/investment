...
export const TASKS: TaskSpec[] = [
  ...
  },
  {
     ...
     *
     * 재무는 분기마다 바뀐다 — 장중에 받을 이유가 없다. 개장 전에 채워 두면
     * 분석가는 DB만 읽는다. **실측: 10분+ → 29.7초, 후보 147종목.**
     *
     * ★ 08:00은 브리핑(08:12)보다 앞이다. 150종목에 2분쯤 걸리고 KIS 호출
     *   사이에 120ms를 쉰다 — 개장 전이라 급할 것이 없다.
     */
    ...
    command: 'cd backend && npx tsx src/scripts/warmFinancialCache.ts --limit 150',
  },
  {
    name: 'deliberate',
    label: '판단자 소집',

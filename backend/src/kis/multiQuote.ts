 ...
 * 준다**(2026-07-31 실측). 그래서 이쪽은 `blank`로 갈라낼 수 있다.
 *
 * ★ **단건 쪽은 2026-08-14에 `getQuote`가 직접 막게 했다.** 그 전까지 여기 주석은
 * *"부르는 쪽이 `price <= 0`을 따로 걸러야 한다"*고 적어 뒀는데, **부르는 쪽이
 * 여럿이라 아무도 안 걸렀다.** 같은 날 폐지 종목(000060)으로 실전·모의 양쪽에서
 * 재 보니 `rt_cd=0` · `정상처리 되었습니다`와 함께 72필드 중 42개가 `'0'`이었다 —
 * 화면은 `0원`이라 적고 그 값이 `estimatedPrice`로 주문 경로까지 갔다.
 * **아는 함정을 호출부에 맡기면 지켜지지 않는다. 값을 만드는 자리에서 막는다.**
 *
 * 여기(멀티)는 `null`로 그 종목만 빼고, 단건은 던진다 — 단건은 그 종목이 전부다.
 *

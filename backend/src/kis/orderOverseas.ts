 ...
 *     미국 매도   실전 TTTT1006U   모의 VTTT1001U ?   ← **확인 안 됐다**
 *
 * ★★ **공식 원문이 스스로 모순이다** (2026-09-11 다시 읽었다). `order.py`의
 *    설명과 주석은 모의 매도를 `VTTT1001U`라 하는데, 같은 파일의 코드는 모의일 때
 *    `tr_id = "V" + tr_id[1:]`로 **`VTTT1006U`를 만든다.** 어느 쪽이 맞는지는
 *    모의에 매도를 보내 봐야만 안다 — 보유 0주에서 보내면 체결될 수 없어 안전하게
 *    가를 수 있지만, 주문 요청이라 사람이 확인하고 한다.
 *
 *    2026-09-10 커밋은 이것을 "짐작했으면 틀렸을 자리"라고 확신했다. 과했다.
 *    그래도 이 표를 **짝마다 명시**하는 것은 맞다 — 확인되면 한 칸만 고치면 된다.
 *
 * ── 지정가만 만든다 ──────────────────────────────────────────────────────
 *
...
const ORDER_TR: Record<OverseasMarket, { prod: TrPair; vts: TrPair }> = {
  US: {
    ...
    // ★ 매도는 미확인 — 공식 설명은 1001U, 공식 코드는 1006U를 만든다(위 주석).

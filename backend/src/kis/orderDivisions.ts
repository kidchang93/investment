/**
 * 국내주식 주문구분(`ORD_DVSN`) 코드.
 *
 * **확인된 것과 아닌 것을 값으로 가른다.** 이 레포는 짐작한 값을 코드에 넣지
 * 않는다(CLAUDE.md). 그런데 주문구분은 짐작이 특히 위험하다 — 틀린 코드로
 * 주문이 나가면 거절되는 데서 끝나지 않고, **다른 뜻으로 접수될 수** 있다.
 *
 * ── 어디까지 확인했나 (2026-08-03) ────────────────────────────────────────
 *
 * 출처는 KIS 공식 저장소 `koreainvestment/open-trading-api`,
 * `legacy/Sample01/kis_domstk.py`다.
 *
 *   54·113행  주문구분 00:지정가, 01:시장가, 02:조건부지정가  **나머지주문구분 API 문서 참조**
 *   376행     00 : 지정가, 01 : 시장가, 02 : 조건부지정가, 03 : 최유리지정가
 *   454·528행 00:지정가, 01:시장가, 02:조건부지정가, **05 : 장전 시간외**
 *   715행     주문단가 … * **장전 시간외, 장후 시간외**, 시장가의 경우 … "0"으로 입력 권고
 *
 * 715행이 **장후 시간외가 존재한다는 것**과 **단가를 0으로 보낸다는 것**까지는
 * 말해 준다. 그런데 그 코드값은 이 저장소 어디에도 없다 — 공식 주석이 스스로
 * "나머지는 API 문서 참조"라고 미룬다. 그 문서는 포털 안에 있고 가져오지 못했다.
 *
 * ★ **지금은 읽힌다.** 개발자센터 주식주문(현금) `ORD_DVSN` 필드가 브라우저 없이
 * JSON으로 온다(`docs/TRADING_API.md` "`ORD_DVSN` 코드" 절에 주소와 전체 표).
 * 2026-09-14부터 `07`(시간외 단일가)이 빠지고 애프터마켓 `41`~`47`이 생겼다 —
 * 애프터마켓 주문 경로를 안 만들었으므로 이 파일에는 넣지 않았다.
 *
 * ── 읽기 전용으로 알아내려다 실패했다 ────────────────────────────────────
 *
 * 매수가능조회(`inquire-psbl-order`)가 `ORD_DVSN`을 받으므로 모르는 코드를
 * 넣어 거절되는지 보면 되겠다고 생각했다. **안 된다** — 대조군으로 넣은 `99`도
 * 정상 응답했다(`probeOrderDivisions.ts` 실측). 그 조회는 주문구분을 검증하지
 * 않고 무시한다. 대조군이 없었다면 "05·06·07 전부 유효"라고 잘못 적을 뻔했다.
 *
 * **남은 확인 방법은 실제 주문 한 건뿐이다.** 모의계좌에서 한 번 내 보면
 * 접수되는지 거절되는지 즉시 안다.
 */

/** 이 값이 무엇인지 **확인된** 코드. 주문 경로에 그대로 써도 된다. */
export const CONFIRMED_ORDER_DIVISIONS = {
  /** 지정가. 이 레포가 실주문으로 확인했다 */
  limit: '00',
  /** 시장가. 이 레포가 실주문으로 확인했다 */
  market: '01',
  /**
   * 장전 시간외(08:30~08:40). KIS 공식 예제가 두 곳에서 그렇게 적는다.
   * **아직 이 레포가 주문으로 확인하지는 않았다** — 문서 근거만 있다.
   */
  preMarketClose: '05',
} as const;

/**
 * 장후 시간외 종가(15:40~16:00)로 **추정하는** 코드. ★ 아직 확인되지 않았다.
 *
 * 확인된 표(`CONFIRMED_ORDER_DIVISIONS`)에 넣지 않은 것은 일부러다. 이 값을 쓰는
 * 경로는 **자기가 미확인 값을 쓰고 있다는 것을 기록에 남겨야 한다.**
 *
 * ── 근거와 그 한계 ────────────────────────────────────────────────────────
 *
 * 공식 예제가 확인해 주는 것은 `05 = 장전 시간외`까지다. 장후 시간외은 존재와
 * "단가 0" 규칙만 적혀 있고(715행) 코드값은 "나머지주문구분 API 문서 참조"로
 * 미뤄져 있다. 이 주석은 처음(08-03)에 `06`이 **05 다음 자리라는 것 외에 근거가
 * 없다**고 적었다.
 *
 * **틀렸다 — 문서 근거가 이미 있었다.** 개발자센터 `ORD_DVSN` 표가 `06 : 장후 시간외`라고
 * 적고, 닷새 전(07-29) `docs/TRADING_API.md`에 옮겨 두었는데 이 주석이 못 봤다
 * (2026-09-17 JSON으로 재확인). 다만 **"종가"라는 말은 없다.**
 * 단일가가 2026-09-14에 폐지돼 남은 장후 시간외가 종가(15:40~16:00)뿐이라는 것은
 * 추론이다. 그리고 **주문으로 접수된 적은 여전히 없다** — 그래서 아래 표로 옮기지 않는다.
 *
 * 읽기 전용으로는 못 가른다 — 매수가능조회는 주문구분을 검증하지 않아 없는
 * 코드(`99`)도 통과한다(`scripts/probeOrderDivisions.ts` 실측).
 *
 * ── 모의계좌로 내 봤다 (2026-08-03 15:56, 장후 시간외 창 안) ─────────────
 *
 * 보유 8종목 전부 이 코드로 매도를 냈고 8건 다 같은 답이 왔다.
 *
 *   40970000 · "모의투자에서 제공하지 않는 주문유형입니다."
 *
 * ★ **이 답이 말해 주는 것과 아닌 것을 가르자.**
 *
 * KIS는 "그런 주문구분은 없다"가 아니라 "그 **주문유형**을 모의가 제공하지
 * 않는다"고 했다. 값을 알아듣고 되돌려준 것이라 `06`이 실재하는 주문구분이라는
 * 쪽을 가리킨다. **그러나 그것이 장후 시간외라는 증명은 아니다** — 모의가 안
 * 받는 다른 주문유형일 수도 있다. (이름은 문서에 있다 — 위 "근거와 그 한계".
 * 남은 것은 실전에서 접수되는지다.)
 *
 * 확정된 것은 하나다: **모의계좌로는 여기까지가 끝이다.** 실전 계좌
 * (`APP_ENV=prod`)에서 한 건 내 봐야 답이 나온다. 그때 접수되면 이 값을 위
 * 표로 옮기고 여기를 지워라.
 */
export const AFTER_HOURS_CLOSE_CANDIDATE = '06';

/**
 * 스톱지정가호가. **스톱가에 닿는 순간 지정가 주문이 나간다.**
 *
 * ── 왜 필요한가 ──────────────────────────────────────────────────────────
 *
 * 감시하는 주체가 **우리가 아니라 거래소**다. 접수된 뒤에도 조건이 주문에 남아
 * 효력 발생을 기다린다 — 조회 응답에 `stpm_efct_occr_yn`(효력발생여부)와
 * `stpm_efct_occr_dtmd`(효력발생상세시각)가 있는 것이 그 증거다. 접수 시점에
 * 소비되는 값이라면 그 두 필드가 존재할 수 없다(`docs/TRADING_API.md`).
 *
 * 그래서 *"러너는 서버 메모리에 살고 재시작하면 멈춘다"*(`docs/ARCHITECTURE.md`)와
 * 부딪히지 않는다. 손절이 우리 프로세스의 수명에 매달려 있지 않다.
 *
 * ── 값은 어디서 왔나 ─────────────────────────────────────────────────────
 *
 * 개발자센터 `주식주문(현금)` 문서의 `ORD_DVSN` 표. 거래소별로 갈려 있고
 * **KRX·NXT는 받고 SOR은 받지 않는다** — 미래에셋이 *"SOR 주문에는 중간가·
 * 스톱지정가호가를 제공하지 않는다"*고 적은 것과 같은 말이다. 서로 다른 두
 * 출처가 일치한다.
 *
 * ── ★ 아직 접수시켜 본 적 없다 ───────────────────────────────────────────
 *
 * 그래서 `CONFIRMED_ORDER_DIVISIONS`에 넣지 않았다. `isUnconfirmedDivision('22')`는
 * **true**이고, 이 값으로 주문을 내는 경로는 그 사실을 기록에 남긴다.
 *
 * 특히 **모의 서버가 이 주문유형을 받는지 모른다.** 문서는 *"모의투자는
 * `EXCG_ID_DVSN_CD`가 KRX만 가능"*이라고만 적었지 주문유형은 말하지 않는다.
 * 안 받으면 `40970000`(*"모의투자에서 제공하지 않는 주문유형입니다"*)으로
 * 돌아온다 — `kis/errorCodes.ts`가 그것을 진짜 거절과 가른다. 접수되면 이
 * 주석을 고치고 확인된 표로 옮겨라.
 */
export const STOP_LIMIT_ORDER_DIVISION = '22';

/**
 * 이 주문구분이 **조건가격(`CNDT_PRIC`)을 요구하는가.**
 *
 * 문서 원문: *"스탑지정가호가 주문 (ORD_DVSN이 22) 사용 시에만 필수"*.
 * `usesZeroPrice`와 짝이다 — 저쪽은 단가를 **비우는** 주문구분, 이쪽은 조건가격을
 * **채워야 하는** 주문구분이다. 스톱지정가는 둘 다 필요하다(지정가 + 스톱가).
 */
export function needsConditionPrice(division: string): boolean {
  return division === STOP_LIMIT_ORDER_DIVISION;
}

/**
 * 이 주문구분이 단가를 비워야 하는가.
 *
 * 공식 예제 715행: *"장전 시간외, 장후 시간외, 시장가의 경우 1주당 가격을
 * 공란으로 비우지 않음 `0`으로 입력 권고"*. 빈 문자열이 아니라 **문자열 `'0'`**이다.
 */
export function usesZeroPrice(division: string): boolean {
  return division === CONFIRMED_ORDER_DIVISIONS.market
    || division === CONFIRMED_ORDER_DIVISIONS.preMarketClose
    || division === AFTER_HOURS_CLOSE_CANDIDATE;
}

/** 이 주문구분이 아직 확인되지 않은 값인가. 기록에 그 사실을 적으려고 쓴다. */
export function isUnconfirmedDivision(division: string): boolean {
  const confirmed: string[] = Object.values(CONFIRMED_ORDER_DIVISIONS);
  return !confirmed.includes(division);
}

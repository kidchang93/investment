/**
 * 층별 장부. **어느 층이 돈을 벌고 어느 층이 까먹는지 가른다.**
 *
 * ── 왜 필요한가 (2026-08-14) ─────────────────────────────────────────────
 *
...
/**
 * 두 층. 값은 DB에 그대로 들어가므로 바꾸면 기록이 갈린다.
 *
 * ── 유망주 층을 없앴다 (2026-09-09) ──────────────────────────────────────
 *
 * 사용자가 정했다 — *"유망주에 대한 언급은 빼고 단기로 다 넣어버리고."*
 *
 * 세 층으로 나눈 목적은 **어느 층이 버는지 가르는 것**이었는데, 25일을 돌려 본
 * 결과 유망주 층과 단기 층이 **같은 방식으로 움직였다.** 둘 다 개별 주식을
 * 뉴스·이벤트로 골라 1~2주 들고 익절·손절로 나온다. 다른 것은 이름뿐이었고,
 * 그 이름 때문에 판단자가 같은 종목을 어느 쪽에 넣을지 매번 고민했다.
 *
 * 나누어 얻은 것보다 잃은 것이 컸다 — **자리 20%가 "강한 증명"을 기다리다
...
export const LAYER_TARGETS: Record<Layer, { weight: number; rationale: string }> = {
  ...
  short: {
    weight: 0.50,
    rationale: '개별 주식. 1~2주 보유, 뉴스·이벤트로 고르고 익절·손절로 나온다',
  },
};
...
export function parseLayer(value: unknown): Layer | null {
  if (value === 'etf' || value === 'short') return value;
  if (value === 'bet') return 'short';
  return null;
}

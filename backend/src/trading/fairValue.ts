 ...
 */
/**
 * 재무 캐시가 신선하다고 보는 시간. **받는 쪽과 읽는 쪽이 같은 값을 써야 한다.**
 *
 * ── 왜 한 곳에 두나 (2026-09-09) ─────────────────────────────────────────
 *
 * `warmFinancialCache.ts`(받는 쪽)와 `analyzeFairValue.ts`(읽는 쪽)에 `12`가
 * 따로 박혀 있었고, 양쪽 주석이 서로 *"같아야 한다"*고 적어 두고 있었다. 같은
 * 파일에서 이미 한 번 그렇게 어긋났었다 — `RELATIVE_CEILING`을 베껴 적었더니
 * 5%와 2%로 갈려 같은 종목이 "비싸다"이면서 추천이었다.
 *
 * ── 왜 12시간에서 7일로 늘렸나 ───────────────────────────────────────────
 *
 * 12시간은 후보가 150종목일 때의 값이다(다시 받아도 3분). 후보를 거래대금
 * 10억 이상 895종목으로 넓히면 **매일 아침 51분**을 다시 받는 값이 된다.
 *
 * ★ 재무는 **분기마다 바뀐다.** 하루에 두 번 다시 받을 이유가 없다. 7일이면
 *   실적이 나온 뒤 늦어도 일주일 안에 반영되고, 그 지연은 분기 실적을 쓰는
 *   적정가에 아무 영향이 없다.
 *
 * ★ 조회 **실패는 캐시하지 않으므로**(받는 쪽 주석 참고) TTL을 늘려도 "지금
 *   못 받은 것"이 일주일 동안 굳지 않는다. 굳는 것은 **빈 응답**뿐이고, 그것은
 *   "이 종목엔 재무가 없다"는 사실이라 일주일 뒤에 물어도 대개 같다.
 */
export const FINANCIAL_TTL_HOURS = 24 * 7;

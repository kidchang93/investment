...
/*
 * 아래 판정문(backtestNote)은 **2026-07-29에 다시 잰 값이다.**
 *
 * 잰 조건: 예수금 50,000원 · 가격·유동성·왕복비용 필터를 통과한 20종목 ·
 * 캔들을 시간순 3구간으로 갈라 잼(**3구간이 가장 최근**) · 매도 거래세 0.20%
 * (왕복 0.43%) · 매수·매도 수수료 각 0.015% · 슬리피지 0.1%.
 *
 * 직전 판정문은 두 겹으로 낡아 있었다 — 세율 0.18%로 쟀고(왕복 0.41%),
 * 백테스트가 **거래별 손익에서 매수 수수료를 안 빼** 승률이 후했다(31b6cd2).
 * 둘 다 고친 뒤 다시 잰 값이다.
 *
 * **승률은 매매 한 건 단위, "플러스 N/20"은 종목 단위다.** 다른 지표라 섞어
 * 읽으면 안 된다.
 *
 * **숫자를 손으로 고치지 않는다** — 판정은 다시 재서 바꾸는 것이지 추정으로
 * 조정하는 게 아니다. `npx tsx src/scripts/measureStrategies.ts 50000 40`으로
 * 다시 잰다. 그 스크립트는 판정을 자동으로 바꾸지 않고 어긋난 것만 알린다.
 */
...
export class MovingAverageCrossStrategy implements Strategy {
  ...
  readonly backtestNote =
    '2026-07-29 재측정(예수금 50,000원으로 실제 매수 가능한 20종목·시간순 3구간):'
...
export class MeanReversionStrategy implements Strategy {
  ...
  readonly backtestNote =
    ...
    + ' 최근으로 올수록 나빠진다. 승률은 70.8%(168회 중 119회)로 셋 중 압도적'
    + ' 1위인데 가장 최근 구간 중앙값은 -0.60%다 — 이기는 횟수는 많고 지는'
    + ' 크기가 크다. 승률만 보고 고르면 안 되는 전형적인 모양이고, 세율을'
    + ' 고치고 매수 수수료까지 제대로 뺀 뒤에도 이 모양은 그대로였다'

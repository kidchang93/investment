...
/**
 * 두 축 판정문에 공통으로 붙는 단서 — **잰 조건이 코드와 달라졌다.**
 *
 * 2026-08-12에 국내 상장 ETF의 매도 거래세 면제를 반영했다(종류 무관). 아래
 * 숫자들은 그 전에, **ETF에도 0.20%를 물려서** 잰 값이다. 표본에 ETF가 섞여
 * 있으므로(`kr-etf`가 표집틀에 있다) 그만큼 나쁘게 적혀 있다.
 *
 * **숫자를 손으로 조정하지 않는다.** 이 레포에서 판정은 다시 재서 바꾸는 것이지
 * 새 사실을 보고 깎는 것이 아니다 — 대신 조건이 달라졌다는 사실을 화면에 적는다.
 */
const ETF_TAX_STALE_NOTE =
  ' ★ 잰 조건이 지금 코드와 다르다: 국내 상장 ETF에도 매도 거래세 0.20%를 물려'
  + ' 잰 값이다(2026-08-12에 면제로 고쳤다). 표본에 ETF가 섞여 있어 그만큼'
  + ' 나쁘게 적혀 있고, 다시 재기 전까지 숫자는 그대로 둔다.';

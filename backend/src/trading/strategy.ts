 ...
 *
 * **2026-08-01에 표본을 8종목 × 4거래일에서 20종목 × 15거래일로 늘렸고 판정은
 * 뒤집히지 않았다.** 이 레포는 8 → 20종목에서 뒤집힌 전례가 있어 그것을 경계해
 * 왔는데, 이번에는 세 전략 모두 방향과 크기가 그대로였다.
 *
 * 다시 재는 경로는 `scripts/collectMinuteCandles.ts` → `measureStrategiesIntraday.ts`다.
 */
const MINUTE_AXIS_SAMPLE =
  '현금 50,000원 · 스크리닝과 같은 순서의 앞 20종목 · 2026-04-29~07-31 15거래일 ·'
  + ' 일봉 고가·저가와 대조해 통과한 249종목·날(51,810봉) · 왕복 비용 0.43% 포함.'
  + ' 시도한 300건 중 51건은 버렸다 — 봉 없음 9 · 고저 불일치 42(대부분 ETF).'
  + ' 8종목 × 4거래일로 재던 것을 늘린 값이고 판정은 뒤집히지 않았다.';
  ...
  readonly measurements = strategyMeasurements(
    {
      ...
      result:
        '매매 1,162회 · 승률 9.81% · 종목 수익률 중앙값 -10.95% · 플러스 1/20종목 ·'
        + ' 비용이 원금의 5.96%(중앙값) · 보유 중앙값 14분.'
        + ' 매수→매도 사이 값 변화가 왕복 비용에 못 미친 매매가 60.2%다.'
        + ' 밤을 넘긴 매매는 6.6%뿐이라 여러 날을 이어 붙인 것이 결과를 흔들지 않았다.'
        + MINUTE_AXIS_WHY,
    },
    {
  ...
  readonly measurements = strategyMeasurements(
    {
      ...
      sample: MINUTE_AXIS_SAMPLE,
      result:
        '매매 3,336회 · 승률 4.38% · 종목 수익률 중앙값 -24.68% · 플러스 0/20종목 ·'
        + ' 비용이 원금의 12.34%(중앙값) · 보유 중앙값 1분.'
        + ' 매수→매도 사이 값 변화가 왕복 비용에 못 미친 매매가 85.2%다 —'
        + ' 20종목 전부에서 잃었고, 매매가 셋 중 가장 잦아 비용이 두 번 깎는다.'
        + MINUTE_AXIS_WHY,
    },
    {
      measuredOn: '2026-07-29',
  ...
  readonly measurements = strategyMeasurements(
    {
      ...
      sample: MINUTE_AXIS_SAMPLE,
      result:
        '매매 1,018회 · 승률 15.03% · 종목 수익률 중앙값 -13.25% · 플러스 1/20종목 ·'
        + ' 비용이 원금의 6.66%(중앙값) · 보유 중앙값 12분.'
        + ' 매수→매도 사이 값 변화가 왕복 비용에 못 미친 매매가 70.7%다.'
        + ' 일봉 축의 승률 70.8%가 이 축에서 15.03%로 내려앉는다 —'

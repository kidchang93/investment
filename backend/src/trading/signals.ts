...
export const SIGNAL_CANDIDATES: SignalCandidate[] = [
  ...
  /*
   * ── 적정가 차트 축 (2026-09-10 추가) ──────────────────────────────────
   *
   * ★★ **새 아이디어가 아니라 이미 돌고 있는 규칙이다.** 2026-09-03부터
   *    `analyzeFairValue.ts`가 이 값으로 후보를 뽑아 판단자에게 올리는데
   *    **원장을 한 번도 거치지 않았다.** 원장 신호 22개가 전부 가격·수급인 것이
   *    우연이 아니라, 재무 시계열이 8분기(약 2년)뿐이라 못 태운 결과였다
   *    (2026-09-10 실측: `getFinancials`가 최대 12를 요청해도 8이 온다).
   *
   * ★ 무엇이 같은가: `chartBand.mid`가 60일 이동평균이고 적정가 gap이
   *   `현재가 / mid − 1`이다. 재무가 없는 ETF는 이것이 gap의 전부이고, 개별
   *   주식도 두 축의 평균이라 절반을 이것이 정한다. 즉 지금 후보 발굴의 주
   *   동력이 **60일선 대비 할인율**이다.
   *
   * ★ **가설을 세운 순서를 밝힌다.** 2026-09-10에 이 축이 뽑은 25칸이 전부
   *   −27~−40% 급락주라는 것을 **먼저 관찰하고 그 뒤에** 재기로 했다. 사전
   *   가설이 아니라 **운영 중인 규칙의 사후 검증**이다 — 원장이 "재기 전에
   *   적는다"를 요구하므로 이 사정을 숨기지 않고 여기 적는다.
   *
   * ★ 계열: `reversal5`(5일)·`rsi14`(14일)·`bollingerB`(20일)와 같은 평균회귀
   *   가족이되 창이 60일로 훨씬 길다. `maCross2060`은 같은 MA60을 쓰지만 방향이
   *   추세추종이라 **반대 가설**이고, 그쪽은 주식에서 한 창도 못 뽑혔다.
   */
  {
    key: 'ma60Discount',
    dataRequirement: 'price',
    frozenAt: '2026-09-10',
    label: '60일선 대비 할인율 (낮을수록 매수)',
    rationale:
      '적정가 차트 축이 실제로 재는 값이다 — chartBand.mid가 60일 이동평균이고'
      + ' gap이 현재가/mid − 1이다. 2026-09-03부터 이것으로 후보를 뽑아 왔으나'
      + ' 원장을 거친 적이 없다. 가설은 "60일 평균 아래로 벌어진 종목이 평균으로'
      + ' 되돌아온다"이고, 반대 가설은 "벌어진 데는 이유가 있고 그 이유가 계속'
      + ' 간다"이다. 짧은 창의 `reversal5`가 t −4.28로 **역방향**이었으므로'
      + ' 후자로 나올 이유가 실제로 있다 — 그렇다면 값이 음수로 나와야 앞뒤가 맞는다.',
    // 60일 평균이므로 index−59…index를 본다 → index가 59는 돼야 한다.
    minHistory: 59,
    score: (ctx) => {
      const mean = windowMean(ctx, ctx.index - 59, ctx.index, (b) => b.close);
      if (mean === undefined || !(mean > 0)) return undefined;
      const { close } = ctx.history[ctx.index];
      if (!(close > 0)) return undefined;
      return -(close / mean - 1);
    },
  },

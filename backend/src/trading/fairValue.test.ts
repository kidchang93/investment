...
suite('합치기', () => {
  ...
  /*
   * 2026-09-09에 후보를 900종목으로 넓히자 ⭐ 추천 다섯이 전부 무너진 종목이었고,
   * 다섯 다 두 축이 2.7~6.2배 어긋나 있었다. 차트는 "이 값이 정상"이라 하고
   * 재무는 "5배는 돼야 한다"고 하는데 그 평균을 적정가라 부르면 −67%가 된다.
   */
  it('★ 두 축이 2배 이상 어긋나면 적정가를 못 낸 것으로 본다', () => {
    // 오가닉티코스메틱: 차트 2,807원 · 재무 17,500원 = 6.2배
    const fv = combine('900300', 'stock', 3_350, band(2_807), band(17_500), []);
    assert.equal(fv.gap, null, '평균 내면 −67%가 되어 순위 맨 위를 차지한다');
    assert.match(fv.missing.join(' '), /어긋나/);
    assert.ok(fv.chart && fv.fundamental, '두 축의 값 자체는 남는다 — 판단자가 본다');
  });

  it('두 축이 2배 안에서 갈리는 것은 정상이다 — 평균을 낸다', () => {
    // 실측 822종목 중 702개(85.4%)가 여기 든다
    const fv = combine('005930', 'stock', 100, band(90), band(160), []);
    assert.ok(fv.gap !== null, '1.8배는 통과해야 한다');
    assert.equal(Math.round(fv.gap! * 1000) / 1000, -0.2, '(90+160)/2 = 125 대비 100');
  });

  it('축이 하나뿐이면 어긋날 짝이 없다 — 검사에 걸리지 않는다', () => {
    // 지수 ETF는 원래 차트 하나로만 낸다
    assert.ok(combine('069500', 'indexEtf', 100, band(1_000), null, []).gap !== null);
  });
});
...
suite('추세 축 — 떨어지는 중인가', () => {
  ...
  it('★★ 종점으로 현재가를 쓴다 — 일봉이 밀리면 옛 종가로 급락을 판정하게 된다', () => {
    /*
     * 2026-09-10 회귀. 판단자가 산수로 짚었다 — SAMG엔터의 마지막 봉이 09-04로
     * 4거래일 밀려 있었고, 그 종가로 재면 시장 대비 −19.91%p라 FALLING_GATE
     * (−20%p)를 **0.09%p 차이로 통과**했다. 현재가로 재면 −25.4%p라 걸린다.
     * 그날 일봉 수집이 도중에 죽어 1,403종목이 그 상태였다.
     */
    const bars = ramp(121, 100, 220);          // 마지막 봉 220 · 60일 전 160
    assert.ok(Math.abs((return60(bars) ?? 0) - 0.375) < 1e-6, '안 주면 예전 그대로');
    // 마지막 봉 뒤로 값이 무너진 경우 — 봉은 그대로인데 현재가만 안다.
    const crashed = return60(bars, 120);
    assert.ok(crashed !== null);
    assert.ok(Math.abs(crashed - (120 / 160 - 1)) < 1e-6, `crashed=${crashed}`);
    assert.ok(crashed < 0, '봉만 보면 +37.5%인데 현재가로는 손실이다');
  });

  it('현재가가 0 이하면 봉으로 되돌아간다 — 시세를 못 받은 것을 하락으로 읽지 않는다', () => {
    const bars = ramp(121, 100, 220);
    assert.equal(return60(bars, 0), return60(bars));
  });

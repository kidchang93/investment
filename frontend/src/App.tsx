      ...
      <header className="app__header">
        ...
        <div className="app__status">
          ...
          */}
          ...
          {/*
            ★ 목표 화면에서는 **주문 게이트 배지 하나만** 남긴다. 아래 것들은 모두
            종목 차트를 볼 때 필요한 값이라(환율·시세 신선도·체결 신선도·시세
            새로고침·시세 WS 연결) 3층 비중을 보는 화면에서는 읽을 일이 없다.
            게이트만 남기는 것은 **실제 돈이 나갈 수 있는 상태인지**는 어느
            화면에서든 보여야 하기 때문이다.
          */}
          {activePage !== 'goal' && (
            <>
          ...
          */}
          {activePage === 'goal' && (
            <>
              ...
              {/*
                ★★ **사무실이 메인 화면이다** (2026-09-07). 사용자가 정했다 —
                   *"이게 메인 화면이면 좋겠어. 지금은 그냥 화면 사이사이 껴 있는
                   정도인 것 같아서 좀 아쉽네."*

                   맞다. 이 앱이 하는 일은 **에이전트가 판단하고 집행하는 것**이고,
                   열자마자 그 장면이 보여야 한다. 자동화 표(작업 17개)가 위에
                   있으면 사무실이 접힌 화면 아래로 밀린다.

                ★ 그렇다고 자동화 카드를 없애지 않는다 — 스위치와 「지금 실행」이
                  거기 있고, "지금 이게 돌고 있나"는 여전히 답해야 하는 질문이다
                  (2026-08-25부터 9일간 안 돌고 있었는데 아무도 몰랐다).
                  사무실 벽 전광판이 그 요약을 이미 이고 있으므로 표는 뒤로 간다.
              */}

...
| **모의 한 달 시험 — 무엇을 언제 판정하나** | **`docs/PAPER_TRIAL_2026_09.md`** |
...
4. **실시간 프레임 파서의 상수를 임의로 바꾸지 말 것.** `H0STCNT0`의 `FIELDS_PER_RECORD = 47`, 필드 인덱스 매핑은 KIS 스펙에 고정돼 있다. **바꿀 때는 실측으로만** 바꾼다 — 2026-09-11에 KIS가 47번째 칸(`MARKET_CLS_CODE`, 애프터마켓 도입)을 붙였고 공식 GitHub 샘플은 46 그대로였다. 46으로 자르던 나흘간 여러 건 붙은 프레임(57%)의 두 번째 건부터 **오류 없이 사라졌다**(2026-09-15 모의·실전 실측). 이제 필드 수가 건수와 안 맞으면 파서가 상태 메시지로 알린다.

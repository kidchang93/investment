...
# ── 절전 차단 ─────────────────────────────────────────────────────────
#
# ★★ **맥이 자면 스케줄러도 잔다** (2026-09-14에 전원 로그로 쟀다).
#
# 2026-09-02에 옛 데몬이 `caffeinate`로 절전을 막게 했는데(`c829c47`), 같은 날
# 스케줄러를 백엔드로 옮기고 9/7에 `daemon.sh`를 지우면서 **그 차단이 함께
# 사라졌다.** 로그가 남은 5거래일이 전부 장중(09:00~15:30)에 잤다 —
#
#   9/7 76분 · 9/8 19분 · 9/9 90분 · 9/10 63분 · 9/11 79분   (`pmset -g log`)
#
# 자는 동안은 **손절 감시도 분석가도 판단자도 없다.** 9/11 14:20~14:36에
# 모든 작업의 하트비트가 17분 비었고, 그 시각이 정확히 `Maintenance Sleep`
# 929초였다. 같은 날 12:43~13:37 공백은 "KIS가 느렸다"로 적었는데 그것도 잠이었다.
#
# ★ `tsx watch`(부모)에 물린다. 코드를 고쳐 재시작해도 부모는 그대로라 차단이
#   이어지고, 백엔드를 내리면(`pkill -f "tsx watch"`) caffeinate도 따라 끝난다 —
#   "멈췄는데 맥만 계속 깨어 있는" 상태가 남지 않는다.
# ★ `-i`(유휴 절전)와 `-s`(시스템 절전, AC에서만 유효)를 함께 준다. 화면은
#   재우게 둔다(`-d`를 안 준다) — 무인 운영에 화면은 필요 없다.
...
if [[ -z "$BACKEND_PID" ]]; then
  ...
  # ★ 변수 뒤에 한글을 바로 붙이면 zsh가 한글까지 이름으로 읽어 `set -u`에 죽는다
  #   (`$BACKEND_PID에: parameter not set` — 처음 쓴 판이 그렇게 여기서 끝났다).
  log "절전 차단 걸려 있음 (백엔드 pid ${BACKEND_PID}에 물림)"
else
  nohup caffeinate -i -s -w "$BACKEND_PID" >/dev/null 2>&1 &
  disown
  sleep 1
  if pgrep -f "^caffeinate -i -s -w ${BACKEND_PID}\$" >/dev/null 2>&1; then
    log "절전 차단 시작 — 백엔드(pid ${BACKEND_PID})가 사는 동안 맥이 유휴로 잠들지 않는다"
  else
    log "★ 절전 차단을 못 걸었다 — 맥이 자면 손절 감시도 멈춘다"
  fi
fi

#!/bin/zsh
# 개장 전 자동 브리핑. **백엔드 스케줄러가 부른다** (평일 08:12~, `backend/src/automation/tasks.ts`).
#
...
#   판단은 판단자(`deliberate.sh`)가 따로 한다.
#
# 쓰는 법:  zsh scripts/premarket.sh
...
#
# ★★ **성공은 남기지 않는다** (2026-09-08). 스케줄러가 작업이 끝나면 같은 이름으로
#    남기므로 **매일 두 줄**이 찍혔다(`premarket` × 2). 판정에 쓰는 것은
#    스케줄러 쪽이고(`doneToday`가 `status='ok'`만 센다), 이쪽은 **실패만** 적어
#    로그 밖에서도 보이게 한다 — 스케줄러는 실패하면 아무것도 안 남긴다.
#
#    ★ `noHeartbeat: true`로 스케줄러를 물러나게 하는 길도 있었지만 **그러면
#      매 분 도는 트랙(A)으로 가서** 창 안에서 계속 돈다. 손절처럼 주기 실행인
#      것에만 쓰는 표시다.
#
...
log "=== 끝 ==="
# 성공은 스케줄러가 남긴다 — 여기서 또 적으면 두 줄이 된다.

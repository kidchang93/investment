...
if [[ $QUICK -eq 1 ]]; then
  ...
  # ── ★ 정식 회차에 먼저 자리를 준다 (2026-09-17) ──
  #
  # 스케줄러는 `deliberate.sh`가 도는 동안 정식 회차를 안 띄운다(`tasks.ts` guard). 그런데
  # 분석가 한 바퀴(2.5~5분)가 계산 한 바퀴(2~4분)보다 길어 이 루프는 거의 쉬지 않는다.
  # 9/17에 첫 계산이 09:07에 끝나 루프가 09:10보다 먼저 떴고, 09:32에 우연히 45초 쉴
  # 때까지 정식 회차가 못 들어갔다. 9/16엔 정식 회차가 09:10:00에 먼저 잡아 괜찮았다 —
  # **누가 먼저 뜨느냐**에 달려 있었다.
  #
  # ★ "오늘 **시작**했나"를 로그로 본다. 하트비트는 성공해야 남으므로 그걸 보면 정식
  #   회차가 실패하는 날 이 루프가 온종일 비켜 선다. 한 번 시작한 뒤로는 락이 가른다.
  # ★ `tasks.ts`의 `deliberate` 창(09:10~)과 맞춘다. 그 작업을 지우면 이것도 지운다.
  if [[ ! "$HM" < "0910" ]] && ! grep -q "] 판단자 소집 · 계좌 $ACCOUNT\$" "$LOG" 2>/dev/null; then
    log "정식 회차가 오늘 아직 시작하지 않았다 — 분석가 루프가 자리를 비킨다"
    exit 0
  fi
fi
START_EPOCH=$(date +%s)

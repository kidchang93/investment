...
20~200배였다. **비율을 말하기 전에 무엇의 비율인지 정해라.**

**8. 표본을 어떻게 골랐는지가 결과의 일부다.** 종목 명단이 `ORDER BY symbol` 앞쪽이면
그건 "시장"이 아니라 "종목코드가 작은 것들"이다. 이걸로 화면 판정문이 **1.4~1.9배
좋게** 적혀 있었고, 분봉·일봉 두 곳에서 각각 한 번씩 나왔다. 명단을 어떻게 뽑았는지
**보고에 반드시 적어라.**

**9. 못 잰 것을 "없다"로 적지 마라.** 표본에 한 건도 없어서 판정 못 한 구간은
**"없다"가 아니라 "못 쟀다"**이다. 500원 미만 종목이 그랬다 — 매매가 11건뿐이었다.
